...
export default function SettingsPage() {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      ...
      loadTenants();
      setTenantMsg({ type: 'success', text: `Workspace has been ${nextStatus === 'active' ? 'activated' : 'deactivated'}.` });
      setTimeout(() => setTenantMsg(null), 3000);
    } catch (err) {
      setTenantMsg({ type: 'error', text: err.response?.data?.message || 'Failed to update workspace status.' });
    }
  };
  ...
  const handleDeleteTenant = async (id, name) => {
    ...
    try {
      ...
      loadTenants();
      setTenantMsg({ type: 'success', text: `Workspace "${name}" has been deleted.` });
      setTimeout(() => setTenantMsg(null), 3000);
    } catch (err) {
      setTenantMsg({ type: 'error', text: err.response?.data?.message || 'Failed to delete workspace.' });
    }
  };
  ...
  return (
    <>
      ...
      {/* ── WORKSPACES & TENANTS MANAGEMENT (Owner only) ── */}
      {user?.role === 'owner' && (
        <div className="card" style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div>
              <div className="card-hd" style={{ margin: 0 }}>🏢 Workspaces &amp; Tenant Management</div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
                Activate, deactivate, or remove tenant workspaces and databases across your platform
              </div>
            </div>
            <a href="/onboard" className="btn btn-primary btn-sm" style={{ textDecoration: 'none' }}>
              ➕ Onboard New Workspace
            </a>
          </div>

          {tenantMsg && (
            <div className={`alert ${tenantMsg.type === 'error' ? 'alert-red' : 'alert-accent'} mb12`}>
              <div className="a-icon">{tenantMsg.type === 'error' ? '⚠️' : '✅'}</div>
              <div><div className="a-title">{tenantMsg.text}</div></div>
            </div>
          )}

          {tenantsLoading ? (
            <div style={{ padding: 20, textAlign: 'center' }} className="spinner">Loading workspaces…</div>
          ) : tenants.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--muted)' }}>No workspaces found.</div>
          ) : (
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {tenants.map(t => {
                  ...
                  return (
                    <tr key={t.id} style={{ opacity: isActive ? 1 : 0.6 }}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          {t.logo_url ? (
                            <img src={t.logo_url} alt="Logo" style={{ height: 28, width: 28, objectFit: 'contain', borderRadius: 4 }} />
                          ) : (
                            <span style={{ fontSize: 20 }}>{t.country === 'SA' ? '🇸🇦' : t.country === 'IN' ? '🇮🇳' : '🏢'}</span>
                          )}
                          <div>
                      ...
                      <td>
                        <span className="badge" style={{ fontSize: 11 }}>
                          {t.country === 'SA' ? '🇸🇦 KSA' : t.country === 'IN' ? '🇮🇳 India' : t.country} · {t.currency}
                        </span>
                      </td>
                      <td>
                        <code style={{ fontSize: 11, color: 'var(--accent)' }}>{t.db_name || 'viva_erp'}</code>
                      </td>
                      <td>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                          ...
                          {isPrimary && (
                            <span className="badge" style={{ fontSize: 10, color: 'var(--muted)' }}>Primary Workspace</span>
                          )}
                        </div>
                      </td>
                    </tr>
